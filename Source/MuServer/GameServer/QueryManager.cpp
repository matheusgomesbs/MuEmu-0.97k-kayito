...
void CQueryManager::Diagnostic() // OK
{
...
bool CQueryManager::ExecQuery(std::string query) // OK
{
	DWORD timeDelay = timeGetTime();

	SQLRETURN result;

	if (SQL_SUCCEEDED((result = SQLExecDirect(this->m_STMT, (SQLCHAR*)query.c_str(), SQL_NTS))) == 0 && result != SQL_NO_DATA)
	{
		this->Diagnostic();
		return 0;
	}
	...
	if ((int)(timeGetTime() - timeDelay) >= 50) {
		gLog.Output(LOG_LUA_EVENT, "[SQL SERVER] delay %d milliseconds", (timeGetTime() - timeDelay));
	}
