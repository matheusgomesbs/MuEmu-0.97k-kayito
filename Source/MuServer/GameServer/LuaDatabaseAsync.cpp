...
#else
void CLuaDatabaseAsync::RegisterFunction(sol::state& lua) {
	lua.set_function("ConnectQueryAsync", &ConnectQueryAsync);
	lua.set_function("CreateAsyncQuery", &CreateAsyncQuery);

	lua.set_function("QueryAsyncGetValue", &QueryAsyncGetValue);
	lua.set_function("QueryAsyncDelete", &QueryAsyncDelete);
}
#endif
