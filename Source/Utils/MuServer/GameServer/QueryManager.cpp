...
{
	if (this->m_SQLEnvironment == SQL_NULL_HANDLE || this->m_SQLConnection == SQL_NULL_HANDLE)
	{
		return 0;
	}
	else
...
{
	gLog.Output(LOG_SCRIPT, "[LuaSQL] %s", query);
	...
	{
		gLog.Output(LOG_SCRIPT, "[LuaSQL] State (%s), Diagnostic: %s", SqlState, MessageText);
	}
