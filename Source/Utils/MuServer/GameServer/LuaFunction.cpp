...
int LuaItemDrop(lua_State* L) // OK
{
	...
	gItemBagManager.DropItemBySpecialValue(dValue, &gObj[aIndex], aValue, bValue, cValue);
...
int LuaItemDropEx(lua_State* L) // OK
{
	...
	int aIndex = lua_tointeger(L, 1); // User index
	int aValue = lua_tointeger(L, 2); // Map
	int bValue = lua_tointeger(L, 3); // Map X
	int cValue = lua_tointeger(L, 4); // Map Y
	int dValue = lua_tointeger(L, 5); // Item Index
	int eValue = lua_tointeger(L, 6); // Item Section
	int fValue = lua_tointeger(L, 7); // Item Level
	int gValue = lua_tointeger(L, 8); // Item Dur
	int hValue = lua_tointeger(L, 9); // Skill Option
	int iValue = lua_tointeger(L, 10); // Luck Option
	int jValue = lua_tointeger(L, 11); // Add Option
	int kValue = lua_tointeger(L, 12); // Loot Index
	int lValue = lua_tointeger(L, 13); // Exe Option
	...
	GDCreateItemSend(aIndex, aValue, bValue, cValue, GET_ITEM(dValue, eValue), fValue, gValue, hValue, iValue, jValue, aIndex, lValue);
...
int LuaItemGive(lua_State* L) // OK
{
	...
	gItemBagManager.DropItemBySpecialValue(aValue, &gObj[aIndex], 0xEB, 0, 0);
...
int LuaItemGiveEx(lua_State* L) // OK
{
	...
	GDCreateItemSend(aIndex, 0xEB, 0, 0, GET_ITEM(dValue, eValue), fValue, gValue, hValue, iValue, jValue, aIndex, lValue);
...
int LuaMessageGet(lua_State* L) // OK
{
	...
	LPOBJ lpObj = &gObj[aValue];

	//gLog.Output(LOG_GENERAL, "%s", lpObj->Lang);

	//strcpy_s(value, gMessage.GetMessage(aValue));
	strcpy_s(value, gMessage.GetTextMessage(bValue, lpObj->Lang));
...
int LuaMessageSend(lua_State* L) // OK
{
	...
	/*gNotice.GCNoticeSend(aIndex, aValue, bValue, "%s", aString);*/
...
int LuaMessageSendToAll(lua_State* L) // OK
{
	...
	for (int n = OBJECT_START_USER; n < MAX_OBJECT; n++)
	{
		if (gObjIsConnectedGP(n) != 0)
		{
			...
			gNotice.GCNoticeSend(n, aValue, "%s", aString);
		}
	}
...
int LuaPostSend(lua_State* L) // OK
{
	...
	{
	case 0:
		PostMessageGold((char*)aString, gServerInfo.m_ServerName, 69, (char*)bString);
		break;
	case 1:
		PostMessageBlue((char*)aString, gServerInfo.m_ServerName, 69, (char*)bString);
		break;
	case 2:
		PostMessageGreen((char*)aString, gServerInfo.m_ServerName, 69, (char*)bString);
		break;
	case 3:
		PostMessageLightGreen((char*)aString, gServerInfo.m_ServerName, 69, (char*)bString);
		break;
	}
