...
bool CCamera3D::MoveMainCamera()
{
	...
	// ViewFar
	{
		...
		CameraViewFar = 1000.0f + CameraDistance * 2.0f + normalizedPitch * 7000.0f;
	}
	...
	if (metodo1)
	{
		float pitchRad = DegToRad(CameraAngle[0]);
		float yawRad = DegToRad(CameraAngle[2]);

		vec3_t PositionTransform = { 0.0f, 0.0f, 0.0f };
		PositionTransform[0] = CameraDistance * sinf(pitchRad) * sinf(yawRad);
		PositionTransform[1] = CameraDistance * sinf(pitchRad) * cosf(yawRad);
		PositionTransform[2] = CameraDistance * cosf(pitchRad);

		vec3_t TargetPosition = { PlayerPosition[0], PlayerPosition[1], PlayerPosition[2] + 450.0f };
...
void CCamera3D::CreateFrustum2D(vec3_t Position)
{
	...
	float fWidthNear = (2.0f * CameraViewNear * tanf(DegToRad(CameraFOV * 0.5f))) + normalizedPitch * 3000.0f;
