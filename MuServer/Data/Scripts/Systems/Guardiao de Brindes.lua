...
--[[
	...
	if Utils.AlterLuaState() ~= "ppborges93@gmail.com" then
		LogAddC(2, string.format("%s nao licenciado para %s", GuardiaoBrindes_Config.Name, Utils.AlterLuaState()))
		auth = false
	end
	
	if GetServerName() ~= "King" then
		LogAddC(2, string.format("%s nao licenciado para %s", GuardiaoBrindes_Config.Name, GetServerName()))
		auth = false
	end
